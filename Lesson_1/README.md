...
При создании жесткого диска выполняется проверка, существует ли диск с таким именем или нет. Если не поставить данную проверку ВМ ломается после нескольких перезагрузок из-за ошибки файла диска. Далее создается SATA контроллер и к нему подключается созданный диск. Также пришлось добавить cd-rom для эксперементов с VboxGuestAdditions.

![screen vagrant](https://github.com/parshyn-dima/screens/blob/master/lesson01/screen-vagrant.png)

Для создания ВМ на основе Vagrantfile необходимо скачать каталог Lesson_1 из репозиторя. Перейти в каталог Lesson_1 и выполнить команду ***vagrant up***.
